...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("State clip");
    group.throughput(Throughput::Elements(1));

    let states = include_str!("../test_data/fixtures/states_source.geojson")
        .parse::<GeoJson>()
        .unwrap();
    let hole_hole = if let GeoJson::FeatureCollection(hole_hole) = states {
        hole_hole
    } else {
