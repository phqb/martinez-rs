...
pub(crate) fn divide_segment(
    ...
    let (mut r, r_id) = SweepEvent::new(p, false, Some(se.id), se.is_subject, None, arena);
    let (mut l, l_id) = SweepEvent::new(p, true, se.other_event, se.is_subject, None, arena);

    r.contour_id = se.contour_id;
    l.contour_id = se.contour_id;

    // avoid a rounding error. The left event would be processed after the right event
    if compare_events(&l, &arena[se.other_event.unwrap()], arena) > Ordering::Equal {
